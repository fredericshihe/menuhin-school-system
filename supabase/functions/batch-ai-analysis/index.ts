...
// 用于在代码逻辑更新后清洗旧分析
// 2026-06-05：AI 周目标改为读取 get_rule_v2_week_target_context，清洗旧目标口径生成的分析
const FORCE_REFRESH_BEFORE = "2026-06-05T14:00:00.000Z";
...
function buildPrompt(student: any, hist: any[], sessions: Session[], targetContext?: any): string {
  // ── 基础变量 ────────────────────────────────────────────────────────────────
  ...
  const remainingWorkdays = todayDOW >= 1 && todayDOW <= 5
    ? Math.max(6 - todayDOW, 1)
    : 0;
  ...
  const storedWScore = Number(student.w_score);
  const isCold     = !!student.is_cold_start;
  const recCnt     = student.record_count ?? 0;

  // ── 维度贡献拆解（按当前规则阶段权重，帮助 AI 解释“为什么是这个分”）──────────
  const activeHistWeeks = (hist || []).filter((x) => Number(x?.composite_score || 0) > 0).length;
  const DIM_WEIGHTS = activeHistWeeks < 4
    ? { B: 0.08, T: 0.05, M: 0.15, A: 0.30, W: 0.42 }
    : activeHistWeeks < 12
      ? { B: 0.12, T: 0.10, M: 0.20, A: 0.30, W: 0.28 }
      : { B: 0.12, T: 0.12, M: 0.24, A: 0.30, W: 0.22 };
  ...
  // ── 各维度白话解释 ────────────────────────────────────────────────────────────
  // W：本周练琴量与后端真实周目标对比；没有 RPC 上下文时才回退到旧近似
  const backendWeeklyTarget = Number(targetContext?.final_target_week);
  const weeklyTarget = Number.isFinite(backendWeeklyTarget) && backendWeeklyTarget > 0
    ? backendWeeklyTarget
    : Math.min(660, Math.max(270, meanDur * 5));
  ...
  const wScoreForExplain = Number.isFinite(storedWScore)
    ? storedWScore
    : wRatio < 0
      ? 0
      : wRatio >= 1
        ? 1
        : 1 / (1 + Math.exp(-3 * (wRatio - 0.5)));
  ...
  // B：短期进步（最近1活跃周 vs 前1活跃周的完成度变化）
  const bText =
    isCold         ? "历史数据还不够，暂时无法比较你最近两周之间的进步情况"
    : bScore > 0.60 ? "最近一周相对前一周更稳或更有进步"
    : bScore < 0.40 ? "最近一周相对前一周有明显回落"
    :                 "最近两周整体基本持平";

  // T：中期趋势（近2活跃周平均完成率 vs 前2活跃周）
  const tText =
    isCold         ? "历史周数还不够（需要至少 3 个有练琴的周），练琴趋势暂时无法评估"
    : tScore > 0.62 ? "近期整体趋势更稳或更强"
    : tScore < 0.38 ? "近期整体趋势有回落"
    :                 "近期整体趋势基本持平";

  // M：稳定性（最近4活跃周稳定达标 + 波动控制）
  const mText =
    isCold         ? "数据还太少，练琴稳定性暂时无法判断"
    : mScore > 0.62 ? "最近几周练琴很规律，大多数周都比较稳定地完成了目标"
    : mScore < 0.38 ? "最近几周练琴不太稳定，周与周之间波动偏大"
    :                 "最近几周练琴稳定性一般，仍有提升空间";

  // A：同专业长期周量水平 + 长期活跃稳定度
  const aText =
    isCold         ? "数据不足，同专业横向积累暂时无法评估"
    : aScore > 0.50 ? "与同专业同学相比，你的长期练琴底盘在中等以上"
    : aScore > 0.20 ? "与同专业同学相比，长期底盘处于中等水平"
    :                 "与同专业同学相比，长期底盘偏弱，需要更持续地练琴来追赶";
  ...
  if (!noPracticeNow && wRatio >= 0 && wRatio < 0.85 && weeklyTarget > 0) {
    const weekGap = Math.max(weeklyTarget - thisWeekValidMin, 0);
    const catchupDays = Math.max(remainingWorkdays, 1);
    actions.push(`W维度（本周量）优先：本周还差约 ${fmtMin(weekGap)} 才到个人周目标，建议按本周剩余 ${catchupDays} 个工作日分摊，平均每天补约 ${fmtMin(Math.max(weekGap / catchupDays, 20))}`);
  }
  ...
  const weekCatchupDays = Math.max(remainingWorkdays, 1);
  const weekCatchupMin = Math.max(Math.round(weekGapMin / weekCatchupDays), 20);
      ...
      action: isMondayNoData
        ...
        : noPracticeNow
          ...
          : weeklyTarget > 0 && wRatio < 0.85
            ? `剩余 ${weekCatchupDays} 个工作日优先补足约 ${fmtMin(weekGapMin)}，平均每天加练约 ${fmtMin(weekCatchupMin)}。`
  ...
  if (diagName) {
    try {
      ...
      const [students, rawSessions, histRows, scheduleRows] = await Promise.all([
        dbGet(`student_baseline?student_name=eq.${encodeURIComponent(diagName)}&select=student_name,composite_score,raw_score,mean_duration,std_duration,alpha,outlier_rate,short_session_rate,record_count,is_cold_start,student_major,student_grade,w_score&limit=1`),
...
`;
  ...
  if (singleName) {
    try {
      ...
      const students = await dbGet(
        `student_baseline?student_name=eq.${encodeURIComponent(singleName)}&select=student_name,composite_score,raw_score,baseline_score,trend_score,momentum_score,accum_score,w_score,mean_duration,record_count,is_cold_start,weeks_improving,personal_best,outlier_rate,short_session_rate,student_major,student_grade&limit=1`
      );
      if (!students.length) {
      ...
      const rawSessionsArr = await dbGet(
        `practice_sessions?student_name=eq.${encodeURIComponent(singleName)}&select=session_start,session_end,cleaned_duration,room_name,is_outlier,outlier_reason&order=session_start.desc&limit=120`
      );
      ...
      let targetContext: any = null;
      try {
        const targetRows = await dbRpc("get_rule_v2_week_target_context", {
          p_student_name: singleName,
          p_week_monday: weekMonday,
        });
        targetContext = Array.isArray(targetRows) ? (targetRows[0] ?? null) : targetRows;
      } catch (e: any) {
        log.push(`⚠️ 周目标上下文读取失败，使用兜底估算：${e.message ?? String(e)}`);
      }
      ...
      const userPrompt = buildPrompt(student, hist, sessions, targetContext);
  ...
  try {
    ...
    const students = await dbGet(
      `student_baseline?select=student_name,composite_score,raw_score,baseline_score,trend_score,momentum_score,accum_score,w_score,mean_duration,record_count,is_cold_start,weeks_improving,personal_best,outlier_rate,short_session_rate,student_major,student_grade&order=composite_score.desc.nullslast,student_name.asc&limit=${pageLimit}&offset=${pageOffset}`
    );
    ...
    async function processOne(student: any): Promise<void> {
      ...
      try {
        ...
        const rawSessions = await dbGet(
          `practice_sessions?student_name=eq.${encodeURIComponent(name)}&select=session_start,session_end,cleaned_duration,room_name,is_outlier,outlier_reason&order=session_start.desc&limit=120`
        );
        ...
        try {
          ...
          log.push(`⚠️ ${name} 周目标上下文读取失败，使用兜底估算：${e.message ?? String(e)}`);
        }
        const weekMondayMs = new Date(weekMonday + "T00:00:00+08:00").getTime();
        // [FIX-19] 与评分系统一致：只有工作日（周一~周五）练琴才算"本周有练琴"
        student.has_week_snapshot = !sessions.some((s) => {
